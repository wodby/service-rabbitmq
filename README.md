# RabbitMQ service for Wodby

Use RabbitMQ as a reusable component in applications managed by Wodby. This
repository contains the service manifests and referenced files used by the
public RabbitMQ service in the Wodby catalog.
...
| Workloads | `main` (StatefulSet), primary; scalable |
| Containers | `rabbitmq` using `wodby/rabbitmq` |
| Endpoints | `rabbitmq`: TCP 5672, HTTP 15672 (main), HTTP 15692 |
| Volumes | Data, 5 GB |
| Helm | chart `oci://registry-1.docker.io/wodby/rabbitmq`; version `0.2.1` |
...
4. Reference `rabbitmq` from your stack manifest.
